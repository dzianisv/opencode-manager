...
async function generateTestAudio(phrase: string): Promise<string | null> {
  ...
  if (process.platform === "darwin") {
    ...
    try {
      ...
      try {
        const picoResult = await execCommand("pico2wave", [
          "-w",
          wavPath,
          phrase,
        ]);
...
async function injectAudioViaWebAPI(
  page: Page,
  audioPath: string,
): Promise<boolean> {
  ...
  try {
    ...
    await page.evaluateOnNewDocument((base64Audio: string) => {
      (
        window as Window & typeof globalThis & { __AUDIO_INJECTED__?: boolean }
      ).__AUDIO_INJECTED__ = false;

      const setupAudioInjection = async () => {
        if (
          (
            window as Window &
              typeof globalThis & { __AUDIO_INJECTED__?: boolean }
          ).__AUDIO_INJECTED__
        ) {
          return;
        }
        ...
        navigator.mediaDevices.getUserMedia = async (constraints) => {
          if (constraints?.audio) {
            console.log(
              "[Test] getUserMedia called with audio - returning injected stream",
            );
            ...
            console.log(
              "[Test] Audio stream started, duration:",
              decodedBuffer.duration,
              "seconds",
            );
    ...
    fail(
      `Audio injection failed: ${error instanceof Error ? error.message : error}`,
    );
    return false;
  }
}

async function waitForVoiceButtonActive(
  page: Page,
  timeoutMs = 30000,
): Promise<boolean> {
  ...
  while (Date.now() - startTime < timeoutMs) {
    const isActive = await page.evaluate(() => {
      ...
      const listeningIndicator = document.querySelector(
        '[class*="bg-green-500"]',
      );
...
async function runBrowserTest(config: TestConfig): Promise<boolean> {
  ...
  console.log(
    `Audio Mode: ${config.useWebAudioInjection ? "Web Audio API injection" : "Chrome fake audio capture"}`,
  );
  console.log("=".repeat(60) + "\n");
  ...
  try {
    ...
    if (repos.error || !repos.length) {
      if (config.sttOnly) {
        info(
          "No repos available - skipping full Talk Mode test in STT-only mode",
        );
        console.log("\n" + "=".repeat(60));
    ...
    info(
      "Creating new session via Node.js fetch (bypassing browser connection pool)...",
    );
    ...
    info(
      "Navigating to session page (using new page to avoid SSE blocking)...",
    );
    ...
    page.on("request", (request) => {
      const url = request.url();
      if (
        url.includes("/api/opencode/event") &&
        !url.includes(encodedRepoPath)
      ) {
        sseBlockCount++;
        if (sseBlockCount <= 3) {
    ...
    if (config.username && config.password) {
      sttHeaders["Authorization"] =
        `Basic ${Buffer.from(`${config.username}:${config.password}`).toString("base64")}`;
    }
    const sttResponse = await fetch(`${config.baseUrl}/api/stt/status`, {
      headers: sttHeaders,
    });
    const sttStatus = (await sttResponse.json()) as {
      server?: { running: boolean; model?: string };
    };
    ...
    success(
      `STT server is running (model: ${sttStatus.server?.model || "unknown"})`,
    );

    info("Checking settings from browser perspective (with cache bypass)...");
    const settingsCheckResponse = await fetch(
      `${config.baseUrl}/api/settings`,
      { headers: sttHeaders },
    );
    const browserSettings = (await settingsCheckResponse.json()) as {
      preferences?: {
        stt?: { enabled?: boolean };
        talkMode?: { enabled?: boolean };
      };
    };
    log(
      `API returns: stt.enabled=${browserSettings?.preferences?.stt?.enabled}, talkMode.enabled=${browserSettings?.preferences?.talkMode?.enabled}`,
      1,
    );
    ...
    await page.evaluate(() => {
      ...
      console.error = (...args: unknown[]) => {
        (
          window as Window & { __consoleErrors?: string[] }
        ).__consoleErrors?.push(args.map(String).join(" "));
    ...
    if (!isListening) {
      ...
      buttonsAfterClick
        .filter((b) => b.title?.toLowerCase().includes("voice"))
        .forEach((b) => log(JSON.stringify(b), 2));
    ...
    if (transcriptionResult) {
      ...
      await page.evaluate(() => {
        ...
        for (const btn of buttons) {
          const title = btn.getAttribute("title")?.toLowerCase() || "";
          if (
            title.includes("stop voice") ||
            title.includes("stop continuous")
          ) {
      ...
      if (!config.sttOnly) {
        ...
        await page.evaluate(
          (text: string, sid: string, repoDir: string) => {
            fetch(
              `/api/opencode/session/${sid}/message?directory=${encodeURIComponent(repoDir)}`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
        ...
        while (Date.now() - responseStartTime < responseMaxWait) {
          const messages = await page.evaluate(
            async (sid: string, dir: string) => {
              try {
                const res = await fetch(
                  `/api/opencode/session/${sid}/message?directory=${encodeURIComponent(dir)}`,
                );
                if (!res.ok) return [];
                return res.json();
              } catch {
            ...
            if (assistantMsgs.length > 0) {
              ...
              for (const part of lastMsg.parts || []) {
                ...
                if (part.type === "tool-result" && part.result) {
                  allText.push(
                    `[Result: ${String(part.result).slice(0, 200)}]`,
                  );
                }
              }
              ...
              if (allText.length > 0) {
                ...
                if (currentResponse !== lastLoggedResponse) {
                  const newContent = currentResponse.slice(
                    lastLoggedResponse.length,
                  );
                  if (newContent.length > 0) {
                    log(
                      `[OpenCode] ${newContent.slice(0, 300)}${newContent.length > 300 ? "..." : ""}`,
                      1,
                    );
                  }
          ...
          if (elapsedSec > 0 && elapsedSec % 15 === 0 && !agentResponse) {
            log(
              `Still waiting for OpenCode response... (${elapsedSec}s elapsed)`,
              1,
            );
          }
    ...
    if (transcriptionResult) {
      ...
      if (sttRequests.length === 0) {
        ...
        sttRequests.forEach((req, i) => {
          log(
            `Request ${i + 1}: ${req.status} - ${req.body?.slice(0, 100) || "no body"}`,
            2,
          );
        });
      }
    ...
    if (passed) {
      ...
      if (config.sttOnly) {
        ...
        console.log(
          "  Real audio -> MediaRecorder -> STT -> Transcription -> OpenCode -> Response",
        );
      } else {
        success("VOICE E2E TEST PASSED");
        console.log("  Real audio -> MediaRecorder -> STT -> Transcription");
        console.log(
          "  Note: OpenCode response was slow/missing (voice pipeline verified)",
        );
      }
      console.log("=".repeat(60));
    } else {
      ...
      fail("TEST FAILED");
      if (!transcribedCorrectly)
        console.log("  - Transcription failed or incorrect");
      console.log("=".repeat(60));
    }
